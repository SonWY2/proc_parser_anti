...
# 플러그인 레지스트리
_registry: Dict[str, Type[MergePlugin]] = {}


def register_plugin(cls: Type[MergePlugin]) -> Type[MergePlugin]:
    """플러그인 등록 데코레이터.
    ...
    """
    if not issubclass(cls, MergePlugin):
        raise TypeError(f"{cls.__name__}은(는) MergePlugin을 상속해야 합니다.")
...
def get_plugin(name: str) -> Type[MergePlugin]:
    """이름으로 플러그인 클래스 조회.
...
def load_plugins(names: Optional[List[str]] = None) -> List[MergePlugin]:
    """플러그인 인스턴스 로드.
...
def load_plugins_by_phase(
    ...
    Args:
        phase: 실행 시점 (PRE_MERGE, POST_MERGE)
        names: 로드할 플러그인 이름 리스트 (None이면 전체 중 필터링)
        
    Returns:
        해당 phase의 플러그인 인스턴스 리스트 (priority 순)
    """
    all_plugins = load_plugins(names)
    
    # phase가 일치하거나 BOTH인 플러그인만 필터링
    filtered = [
        p for p in all_plugins 
        if p.phase == phase or p.phase == PluginPhase.BOTH
    ]
