...
class ParseProcCodeSkill(BaseSkill):
    ...
    def _get_parser(self):
        ...
        if self._parser is None:
            try:
                parser_module = importlib.import_module("parsing.core")
                self._parser = parser_module.ProCParser()
    ...
    def _parse_from_string(self, parser: Any, source_code: str) -> List[Dict[str, Any]]:
        """
        문자열에서 직접 파싱
    ...
    def _structure_elements(self, elements: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        파싱된 요소를 카테고리별로 구조화
        ...
        for elem in elements:
            ...
            if elem_type == "include":
                ...
                result["unknown_segments"].append(
                    {
